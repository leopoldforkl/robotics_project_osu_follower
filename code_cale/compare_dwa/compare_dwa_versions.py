...
def run_dwa_version(dwa_control, motion, get_goal, offset_dist, offset_angle_rad, tag):
    ...
    for step in range(ARC_TOTAL_STEPS + 1):
        ...
        corrected_target_path.append(goal_pos)
    ...
    return np.array(trajectory), np.array(guide_trail), np.array(distance_errors), np.rad2deg(np.array(angle_errors)), np.array(corrected_target_path)
...
def main(offset_dist=2.0):
    ...
    for offset_angle_deg in test_angles:
        ...
        traj_v1, guide_v1, dist_err_v1, ang_err_v1, goals_v1 = run_dwa_version(dwa_v1, motion_v1, get_displaced_zero_goal, offset_dist, offset_angle_rad, tag='2')
        traj_v2, guide_v2, dist_err_v2, ang_err_v2, goals_v2 = run_dwa_version(dwa_v2, motion_v2, get_displaced_zero_goal, offset_dist, offset_angle_rad, tag='3')
        traj_v3, guide_v3, dist_err_v3, ang_err_v3, goals_v3 = run_dwa_version(dwa_v3, motion_v3, get_displaced_zero_goal, offset_dist, offset_angle_rad, tag='4')
        ...
        planner_trajs = [
            (traj_v1, goals_v1, "Naive DWA", "r", "summary_v1"),
            (traj_v2, goals_v2, "Orientation-Aware DWA", "b", "summary_v2"),
            (traj_v3, goals_v3, "Hybrid DWA-GA", "g", "summary_v3"),
        ]

        for traj, goal_path, label, color, fname in planner_trajs:
            plt.figure(figsize=(10, 8))
            plt.plot(traj[:, 0], traj[:, 1], color + "-", label=label)
            plt.plot(guide_v1[:, 0], guide_v1[:, 1], "k--", label="Target Path")

            for j in range(0, min(len(traj), len(goal_path)), 25):
                dx = goal_path[j, 0] - traj[j, 0]
                dy = goal_path[j, 1] - traj[j, 1]
                plt.quiver(traj[j, 0], traj[j, 1], dx, dy, angles='xy', scale_units='xy', scale=2.0,
        ...
        for err, ylabel, title, fname in [
            ((dist_err_v1, dist_err_v2, dist_err_v3), "Distance Error (m)", "Distance Error Over Time", "distance_error_time_"),
            ((np.abs(ang_err_v1), np.abs(ang_err_v2), np.abs(ang_err_v3)), "Angle Error (deg)", "Angle Error Over Time", "angle_error_time_"),
        ]:
            plt.figure(figsize=(10, 5))
            plt.plot(err[0], label="Naive DWA")
            plt.plot(err[1], label="Orientation-Aware DWA")
            plt.plot(err[2], label="Hybrid DWA-GA")
            plt.xlabel("Timestep", fontsize=24)
            plt.ylabel(ylabel, fontsize=18)
            plt.title(f"{title} - Angle {offset_angle_deg}°", fontsize=20)
            plt.legend(prop={'size': 16})
            plt.tight_layout()
            plt.savefig(f"compare_dwa/plots/{fname}{offset_angle_deg}.svg", format='svg')
            plt.close()
