...
def get_displaced_goal(step, offset_dist, offset_angle_rad, lookahead_steps=0):
    """
    Computes a displaced goal from a projected future point on a circular arc.
    The displacement is applied at an angle relative to the arc tangent.
    """
...
def calc_relative_angle_cost(final_pose, goal, desired_angle_rad):
    """
    Penalize deviation from desired relative angle to the goal.
    final_pose: [x, y, yaw]
    goal: [x, y]
    """
    ...
    # Penalize deviation from desired relative viewing angle
...
def get_guide_point(step):
    """
    Returns the raw point on the circular arc before offset is applied.
    """
...
def compute_angle_error(robot_pose, goal, desired_angle_rad):
    """
    Computes the difference between the robot's current relative angle to the goal
    and the desired relative angle.
    """
